...
namespace Cavrnus.SpatialConnector.Samples.MobileDemo
{
    public class MobileUserListEntry : MonoBehaviour
    {
		...
		private IEnumerator AssignVidTexture(TextureWithUVs tex)
		{
			...
			yield return StartCoroutine(WaitForTextureReady(tex.Texture));
			
			videoStreamImage.texture = tex.Texture;
			videoStreamImage.uvRect = tex.UVRect;
		}
		
		private IEnumerator WaitForTextureReady(Texture texture)
		{
			var timeout = 2.0f;
			var elapsed = 0.0f;
			...
			{
				if (texture != null && texture.width > 0 && texture.height > 0 && texture.isReadable)
					yield break;
